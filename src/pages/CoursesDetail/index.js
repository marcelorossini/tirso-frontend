...
export default ({ history, match }) => {
  ...
  useEffect(() => {
    ...
    const getCourseDetail = async () => {
      ...
      if (course.data.length === 0) history.push("/");
  ...
  const handleSendForm = async () => {
    ...
    if (phone.value.length === 0) {
      ...
      inputError(
        phone,
        false,
        "Verifique se o nome foi informado corretamente!"
      );
    }
  ...
  return (
    <>
      <Helmet>
        <title>CURSO {courseDetail.title || ''} - Tirso Cursos e Treinamentos -</title>
      </Helmet>
      <Wrapper
        isLoaded={isLoaded}
        ...
        <div className="course-detail">
          <div>
            <div className="card">
              ...
              <div className="subtitle">{courseDetail.subtitle}</div>
              <div className="video">
                {(courseDetail.video || "") !== "" ? (
                  <div className="video-wrapper">
                    <iframe
                      title="video"
                      src={`//www.youtube.com/embed/${courseDetail.video}?autoplay=1`}
                      frameBorder="0"
                      allowFullScreen
                    ></iframe>
                  </div>
              ...
              >
                <input
                  ...
                  onKeyPress={(e) => {
                    if (
                      !/^[A-Za-záàâãéèêíïóôõöúçñÁÀÂÃÉÈÍÏÓÔÕÖÚÇÑ ]+$/.test(e.key)
                    )
                      e.preventDefault();
                  }}
                ></input>
